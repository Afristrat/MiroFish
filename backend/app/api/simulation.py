...
def get_graph_entities(graph_id: str):
    ...
    try:
        if not Config.ZEP_API_KEY:
            return jsonify({
                "success": False,
                "error": "ZEP_API_KEY non configuré"
            }), 500
...
def create_simulation():
    ...
    try:
        ...
        if not project_id:
            return jsonify({
                "success": False,
                "error": "Veuillez fournir le project_id"
            }), 400
        ...
        if not graph_id:
            return jsonify({
                "success": False,
                "error": "Le graphe du projet n'a pas encore été construit, veuillez d'abord appeler /api/graph/build"
            }), 400
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    try:
        ...
        if status in prepared_statuses and config_generated:
            ...
            return False, {
                "reason": f"L'état n'est pas dans la liste des états prêts ou config_generated est false : status={status}, config_generated={config_generated}",
                "status": status,
                "config_generated": config_generated
            }
...
def prepare_simulation():
    ...
    try:
        ...
        if not simulation_id:
            return jsonify({
                "success": False,
                "error": "Veuillez fournir le simulation_id"
            }), 400
            ...
            if is_prepared:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "message": "La préparation est déjà terminée, pas besoin de régénérer",
        ...
        if not simulation_requirement:
            return jsonify({
                "success": False,
                "error": "Description des besoins de simulation manquante dans le projet (simulation_requirement)"
            }), 400
        ...
        def run_prepare():
            try:
                task_manager.update_task(
                    ...
                    progress=0,
                    message="Démarrage de la préparation de l'environnement de simulation..."
                )
                ...
                def progress_callback(stage, progress, message, **kwargs):
                    ...
                    stage_names = {
                        ...
                        "generating_profiles": "Génération des profils d'agents",
        ...
        return jsonify({
            ...
            "data": {
                ...
                "task_id": task_id,
                "status": "preparing",
                "message": "La tâche de préparation a été lancée, veuillez consulter la progression via /api/simulation/prepare/status",
            ...
            if is_prepared:
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "progress": 100,
                        "message": "La préparation est déjà terminée",
                        "already_prepared": True,
                        "prepare_info": prepare_info
                    }
                })
            ...
            if simulation_id:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "not_started",
                        "progress": 0,
                        "message": "La préparation n'a pas encore commencé, veuillez appeler /api/simulation/prepare",
                        "already_prepared": False
                    }
                })
            return jsonify({
                "success": False,
                "error": "Veuillez fournir le task_id ou le simulation_id"
            }), 400
                ...
                if is_prepared:
                    return jsonify({
                        ...
                        "data": {
                            ...
                            "status": "ready",
                            "progress": 100,
                            "message": "Tâche terminée (la préparation existe déjà)",
                            "already_prepared": True,
                            "prepare_info": prepare_info
                        }
                    })
...
def generate_profiles():
    ...
    try:
        ...
        if not graph_id:
            return jsonify({
                "success": False,
                "error": "Veuillez fournir le graph_id"
            }), 400
        ...
        if filtered.filtered_count == 0:
            return jsonify({
                "success": False,
                "error": "Aucune entité correspondant aux critères n'a été trouvée"
            }), 400
    ...
    try:
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                ...
                if state.status == SimulationStatus.RUNNING:
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            ...
                            return jsonify({
                                "success": False,
                                "error": f"La simulation est en cours d'exécution, veuillez d'abord appeler /stop pour l'arrêter, ou utilisez force=true pour forcer le redémarrage"
                            }), 400
                ...
                return jsonify({
                    "success": False,
                    "error": f"La simulation n'est pas prête, état actuel : {state.status.value}, veuillez d'abord appeler /prepare"
                }), 400
        ...
        if enable_graph_memory_update:
            ...
            if not graph_id:
                return jsonify({
                    "success": False,
                    "error": "La mise à jour de la mémoire du graphe nécessite un graph_id valide, veuillez vous assurer que le graphe du projet a été construit"
                }), 400
...
def interview_agent():
    ...
    try:
        ...
        if agent_id is None:
            return jsonify({
                "success": False,
                "error": "Veuillez fournir le agent_id"
            }), 400
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "L'environnement de simulation n'est pas en cours d'exécution ou a été fermé. Veuillez vous assurer que la simulation est terminée et en mode attente de commandes."
            }), 400
        ...
        return jsonify({
            "success": False,
            "error": f"Délai d'attente de la réponse d'entretien dépassé : {str(e)}"
        }), 504
...
def interview_agents_batch():
    ...
    try:
        ...
        if not interviews or not isinstance(interviews, list):
            return jsonify({
                "success": False,
                "error": "Veuillez fournir les interviews (liste des entretiens)"
            }), 400
        ...
        for i, interview in enumerate(interviews):
            if 'agent_id' not in interview:
                return jsonify({
                    "success": False,
                    "error": f"L'élément {i+1} de la liste des entretiens ne contient pas de agent_id"
                }), 400
            if 'prompt' not in interview:
                return jsonify({
                    "success": False,
                    "error": f"L'élément {i+1} de la liste des entretiens ne contient pas de prompt"
                }), 400
            # 验证每项的platform（如果有）
            ...
            if item_platform and item_platform not in ("twitter", "reddit"):
                return jsonify({
                    "success": False,
                    "error": f"Le paramètre platform de l'élément {i+1} ne peut être que 'twitter' ou 'reddit'"
                }), 400
        ...
        return jsonify({
            "success": False,
            "error": f"Délai d'attente des réponses d'entretien en lot dépassé : {str(e)}"
        }), 504
...
def interview_all_agents():
    ...
    try:
        ...
        return jsonify({
            "success": False,
            "error": f"Délai d'attente de la réponse d'entretien global dépassé : {str(e)}"
        }), 504
...
def get_env_status():
    ...
    try:
        ...
        if env_alive:
            message = "L'environnement est en cours d'exécution, prêt à recevoir des commandes d'entretien"
        else:
            message = "L'environnement n'est pas en cours d'exécution ou a été fermé"

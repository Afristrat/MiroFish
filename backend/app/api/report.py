...
def generate_report():
    ...
    try:
        ...
        if not graph_id:
            return jsonify({
                "success": False,
                "error": "ID du graphe manquant, veuillez vous assurer que le graphe a été construit"
            }), 400
        ...
        if not simulation_requirement:
            return jsonify({
                "success": False,
                "error": "Description des besoins de simulation manquante"
            }), 400
        ...
        def run_generate():
            try:
                task_manager.update_task(
                    ...
                    progress=0,
                    message="Initialisation du Report Agent..."
                )
        ...
        return jsonify({
            ...
            "data": {
                ...
                "task_id": task_id,
                "status": "generating",
                "message": "La tâche de génération du rapport a été lancée, veuillez consulter la progression via /api/report/generate/status",
            ...
            if existing_report and existing_report.status == ReportStatus.COMPLETED:
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "completed",
                        "progress": 100,
                        "message": "Le rapport a été généré",
...
def get_report(report_id: str):
    ...
    try:
        ...
        if not report:
            return jsonify({
                "success": False,
                "error": f"Le rapport n'existe pas : {report_id}"
            }), 404
...
def get_report_by_simulation(simulation_id: str):
    ...
    try:
        ...
        if not report:
            return jsonify({
                "success": False,
                "error": f"Aucun rapport disponible pour cette simulation : {simulation_id}",
                "has_report": False
            }), 404
...
def delete_report(report_id: str):
    ...
    try:
        ...
        return jsonify({
            "success": True,
            "message": f"Rapport supprimé : {report_id}"
        })
...
def chat_with_report_agent():
    ...
    try:
        ...
        if not message:
            return jsonify({
                "success": False,
                "error": "Veuillez fournir le message"
            }), 400
...
def get_report_progress(report_id: str):
    ...
    try:
        ...
        if not progress:
            return jsonify({
                "success": False,
                "error": f"Le rapport n'existe pas ou les informations de progression sont indisponibles : {report_id}"
            }), 404
...
def get_single_section(report_id: str, section_index: int):
    ...
    try:
        ...
        if not os.path.exists(section_path):
            return jsonify({
                "success": False,
                "error": f"La section n'existe pas : section_{section_index:02d}.md"
            }), 404
...
def search_graph_tool():
    ...
    try:
        ...
        if not graph_id or not query:
            return jsonify({
                "success": False,
                "error": "Veuillez fournir le graph_id et la query"
            }), 400

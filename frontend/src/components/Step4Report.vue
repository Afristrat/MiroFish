    ...
    <div class="main-split-layout">
            ...
            >
              ...
              <div class="section-body" v-show="!collapsedSections.has(idx)">
                ...
                <div v-else-if="currentSectionIndex === idx + 1" class="loading-state">
                  ...
                  <span class="loading-text">Génération en cours : {{ section.title }}...</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="right-panel" ref="rightPanel">
        ...
        <div class="workflow-overview" v-if="agentLogs.length > 0 || reportOutline">
          ...
          <!-- Next Step Button - Affiché après la complétion -->
            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (contenu généré, mais la section entière peut ne pas être terminée) -->
                  ...
                  <!-- Section Complete (génération de la section terminée) -->
                  ...
                  <template v-if="log.action === 'llm_response'">
                    ...
                    <!-- Afficher un indicateur spécial pour la réponse finale -->
...
const toggleRawResult = (timestamp, event) => {
  // Sauvegarder la position du bouton par rapport au viewport
  ...
  // Après la mise à jour du DOM, ajuster la position de défilement pour maintenir le bouton à la même position
...
const toggleSectionCollapse = (idx) => {
  // Seules les sections terminées peuvent être repliées
  if (!generatedSections.value[idx + 1]) return
...
const toolConfig = {
  ...
  'interview_agents': {
    ...
    color: 'green',
    icon: 'users' // Icône utilisateurs - représente les conversations
  },
  'quick_search': {
    name: 'Quick Search',
    color: 'orange',
...
const parseInsightForge = (text) => {
  ...
  try {
    // Extraire la question d'analyse
    const queryMatch = text.match(/分析问题:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()

    // Extraire le scénario de prédiction
    const reqMatch = text.match(/预测场景:\s*(.+?)(?:\n|$)/)
    if (reqMatch) result.simulationRequirement = reqMatch[1].trim()

    // Extraire les statistiques - correspondance au format "相关预测事实: X条"
    ...
    // Extraire les sous-questions - extraction complète, sans limite
    ...
    // Extraire les faits clés - extraction complète, sans limite
    ...
    // Extraire les entités principales - extraction complète, incluant résumé et nombre de faits associés
    const entitySection = text.match(/### 【核心实体】\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
      const entityText = entitySection[1]
      // Découper les blocs d'entités par "- **"
    ...
    // Extraire les chaînes de relations - extraction complète, sans limite
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Extraire les faits actifs actuels - extraction complète, sans limite
    const activeSection = text.match(/### 【当前有效事实】[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (activeSection) {
      const lines = activeSection[1].split('\n').filter(l => l.match(/^\d+\./))
      result.activeFacts = lines.map(l => {
        // Supprimer les numéros et guillemets
        const factText = l.replace(/^\d+\.\s*/, '').replace(/^"|"$/g, '').trim()
        return factText
      }).filter(Boolean)
    }
    
    // Extraire les faits historiques/expirés - extraction complète, sans limite
    ...
    // Extraire les entités impliquées - extraction complète, sans limite
...
const parseInterview = (text) => {
  ...
  try {
    // Extraire le sujet d'entretien
    const topicMatch = text.match(/\*\*采访主题:\*\*\s*(.+?)(?:\n|$)/)
    if (topicMatch) result.topic = topicMatch[1].trim()
    
    // Extraire le nombre de personnes interviewées (ex : "5 / 9 Agents simulés")
    ...
    // Extraire les raisons du choix des interviewés
    const reasonMatch = text.match(/### 采访对象选择理由\n([\s\S]*?)(?=\n---\n|\n### 采访实录)/)
    if (reasonMatch) {
      result.selectionReason = reasonMatch[1].trim()
    }
    
    // Analyser les raisons de sélection de chaque personne
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // Format 1 : numéro. **nom（index=X）** : raison
        // Exemple : 1. **校友_345（index=1）** : en tant qu'ancien élève...
        ...
        // Format 2 : - sélection nom（index X） : raison
        // Exemple : - 选择家长_601（index 0） : en tant que représentant des parents...
        if (!headerMatch) {
        ...
        // Format 3 : - **nom（index X）** : raison
        // Exemple : - **家长_601（index 0）** : en tant que représentant des parents...
        if (!headerMatch) {
        ...
        if (name) {
          // Sauvegarder la raison de la personne précédente
          if (currentName && currentReason.length > 0) {
            reasons[currentName] = currentReason.join(' ').trim()
          }
          // Commencer une nouvelle personne
          currentName = name
          currentReason = reasonStart ? [reasonStart.trim()] : []
        } else if (currentName && line.trim() && !line.match(/^未选|^综上|^最终选择/)) {
          // Suite de la raison (exclure les paragraphes de résumé final)
          currentReason.push(line.trim())
        }
      }
      
      // Sauvegarder la raison de la dernière personne
      if (currentName && currentReason.length > 0) {
        reasons[currentName] = currentReason.join(' ').trim()
      }
    ...
    // Extraire chaque enregistrement d'entretien
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // Extraire le titre (ex : "étudiant", "professionnel de l'éducation", etc.)
      ...
      if (nameRoleMatch) {
        ...
        // Définir la raison de sélection de cette personne
      ...
      // Extraire la liste de questions
      const qMatch = block.match(/\*\*Q:\*\*\s*([\s\S]*?)(?=\n\n\*\*A:\*\*|\*\*A:\*\*)/)
      if (qMatch) {
        const qText = qMatch[1].trim()
        // Découper les questions par numérotation
        const questions = qText.split(/\n\d+\.\s+/).filter(q => q.trim())
        if (questions.length > 0) {
          // Si la première question est précédée de "1.", traitement spécial
      ...
      // Extraire les réponses - séparées par Twitter et Reddit
      const answerMatch = block.match(/\*\*A:\*\*\s*([\s\S]*?)(?=\*\*关键引言|$)/)
      if (answerMatch) {
        const answerText = answerMatch[1].trim()
        
        // Séparer les réponses Twitter et Reddit
        ...
        // Logique de repli de plateforme (compatibilité ancien format : un seul marqueur de plateforme)
        if (!twitterMatch && redditMatch) {
          // Uniquement réponse Reddit, copier comme affichage par défaut si ce n'est pas un texte fictif
          ...
          // Pas de marqueur de plateforme (très ancien format), traiter l'ensemble comme réponse
          interview.twitterAnswer = answerText
        }
      }
      
      // Extraire les citations clés (compatible avec plusieurs formats de guillemets)
      const quotesMatch = block.match(/\*\*关键引言:\*\*\n([\s\S]*?)(?=\n---|\n####|$)/)
      if (quotesMatch) {
        const quotesText = quotesMatch[1]
        // Correspondance prioritaire au format > "text"
        let quoteMatches = quotesText.match(/> "([^"]+)"/g)
        // Repli : correspondance > "text" ou > \u201Ctext\u201D (guillemets chinois)
        if (!quoteMatches) {
    ...
    // Extraire le résumé des entretiens
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    // Extraire les faits pertinents - extraction complète, sans limite
    ...
    // Tenter d'extraire les informations sur les arêtes (si disponibles)
    ...
    // Tenter d'extraire les informations sur les nœuds (si disponibles)
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-tabs' }, [
        ...
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `Entités principales (${props.result.entities.length})`)
        ]),
        h('button', {
          class: ['insight-tab', { active: activeTab.value === 'relations' }],
          onClick: () => { activeTab.value = 'relations' }
        }, [
          h('span', { class: 'tab-label' }, `Chaînes de relations (${props.result.relations.length})`)
        ]),
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Derniers faits clés associés dans la mémoire temporelle'),
            h('span', { class: 'panel-count' }, `${props.result.facts.length} entrées au total`)
          ]),
          ...
          }, expandedFacts.value ? `Replier ▲` : `Déplier tout ${props.result.facts.length} entrées ▼`)
        ]),
        
        // Entities Tab
        activeTab.value === 'entities' && props.result.entities.length > 0 && h('div', { class: 'entities-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Entités principales'),
            h('span', { class: 'panel-count' }, `${props.result.entities.length} entités au total`)
          ]),
          h('div', { class: 'entities-grid' },
            (expandedEntities.value ? props.result.entities : props.result.entities.slice(0, 12)).map((entity, i) => 
              h('div', { class: 'entity-tag', key: i, title: entity.summary || '' }, [
                ...
                entity.relatedFactsCount > 0 && h('span', { class: 'entity-fact-count' }, `${entity.relatedFactsCount} entrées`)
              ])
            )
          ),
          ...
          }, expandedEntities.value ? `Replier ▲` : `Déplier tout ${props.result.entities.length} entités ▼`)
        ]),
        
        // Relations Tab
        activeTab.value === 'relations' && props.result.relations.length > 0 && h('div', { class: 'relations-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Chaînes de relations'),
            h('span', { class: 'panel-count' }, `${props.result.relations.length} entrées au total`)
          ]),
          ...
          }, expandedRelations.value ? `Replier ▲` : `Déplier tout ${props.result.relations.length} entrées ▼`)
        ]),
        
        // Sub-queries Tab
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Sous-questions d\'analyse générées par requête dérivée'),
            h('span', { class: 'panel-count' }, `${props.result.subQueries.length} au total`)
          ]),
        ...
        activeTab.value === 'entities' && props.result.entities.length === 0 && h('div', { class: 'empty-state' }, 'Aucune entité principale'),
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `Mémoire active actuelle (${props.result.activeFacts.length})`)
        ]),
        h('button', {
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Mémoire active actuelle'),
            h('span', { class: 'panel-count' }, `${props.result.activeFacts.length} entrées au total`)
          ]),
          ...
          ) : h('div', { class: 'empty-state' }, 'Aucune mémoire active actuelle'),
          props.result.activeFacts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedActive.value = !expandedActive.value }
          }, expandedActive.value ? `Replier ▲` : `Déplier tout ${props.result.activeFacts.length} entrées ▼`)
        ]),
        ...
        activeTab.value === 'historical' && h('div', { class: 'facts-panel historical-facts' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `${props.result.historicalFacts.length} entrées au total`)
          ]),
            ...
            (expandedHistorical.value ? props.result.historicalFacts : props.result.historicalFacts.slice(0, INITIAL_SHOW_COUNT)).map((fact, i) => 
              h('div', { class: 'fact-item historical', key: i }, [
                ...
                h('div', { class: 'fact-content' }, [
                  // Tenter d'extraire les informations temporelles [time - time]
                  (() => {
          ...
          ) : h('div', { class: 'empty-state' }, 'Aucune mémoire historique'),
          props.result.historicalFacts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedHistorical.value = !expandedHistorical.value }
          }, expandedHistorical.value ? `Replier ▲` : `Déplier tout ${props.result.historicalFacts.length} entrées ▼`)
        ]),
        ...
        activeTab.value === 'entities' && h('div', { class: 'entities-panel' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'Aucune entité impliquée'),
...
// Composant d'affichage des entretiens - Conversation Style (Q&A Format)
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Maintenir un état de sélection de plateforme indépendant pour chaque paire question-réponse
    const platformTabs = reactive({}) // { 'agentIdx-qIdx': 'twitter' | 'reddit' }
    
    // Obtenir la sélection de plateforme actuelle pour une question
    const getPlatformTab = (agentIdx, qIdx) => {
      const key = `${agentIdx}-${qIdx}`
      return platformTabs[key] || 'twitter'
    }
    
    // Définir la sélection de plateforme pour une question
    ...
    // Vérifier si c'est un texte fictif de plateforme
    ...
    // Tenter de découper les réponses par numéro de question
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // Prise en charge de deux formats de numérotation :
      // 1. "问题X：" ou "问题X:" (format chinois, nouveau format backend)
      // 2. "1. " ou "\n1. " (numéro+point, compatibilité ancien format)
      let matches = []
      let match

      // Essayer en priorité le format "问题X："
      const cnPattern = /(?:^|[\r\n]+)问题(\d+)[：:]\s*/g
      ...
      // Si pas de correspondance, repli vers le format "numéro."
      if (matches.length === 0) {
        const numPattern = /(?:^|[\r\n]+)(\d+)\.\s+/g
      ...
      // Si aucun numéro trouvé ou un seul, retourner l'ensemble
      if (matches.length <= 1) {
      ...
      // Extraire chaque partie par numéro
      const parts = []
    ...
    // Obtenir la réponse correspondant à une question
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // Découpage réussi et index valide
      if (answers.length > 1 && qIdx < answers.length) {
        return answers[qIdx] || ''
      }

      // Échec du découpage : la première question retourne la réponse complète, les autres retournent vide
      return qIdx === 0 ? answer : ''
    }
    
    // Vérifier si une question a des réponses sur deux plateformes (filtrer les textes fictifs)
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // Les deux plateformes ont de vraies réponses (non fictives) et le contenu diffère
    ...
    return () => h('div', { class: 'interview-display' }, [
      ...
      props.result.interviews.length > 0 && h('div', { class: 'interview-detail' }, [
        ...
        // Selection Reason - Raison de sélection
        ...
        // Q&A Conversation Thread - style question-réponse
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Boutons de bascule entre plateformes (affiché uniquement si les deux plateformes ont de vraies réponses)
                  ...
                  // Expand/Collapse Button (non affiché pour les textes fictifs)
      ...
      h('div', { class: ['quicksearch-content', { 'no-tabs': !showTabs.value }] }, [
        ...
        activeTab.value === 'edges' && hasEdges.value && h('div', { class: 'edges-panel' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `${props.result.edges.length} entrées au total`)
          ]),
        ...
        activeTab.value === 'nodes' && hasNodes.value && h('div', { class: 'nodes-panel' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `${props.result.nodes.length} nœuds au total`)
          ]),
...
// Étape active actuelle (pour l'affichage en haut)
const activeStep = computed(() => {
  const steps = workflowSteps.value
  // Trouver l'étape active actuelle
  const active = steps.find(s => s.status === 'active')
  if (active) return active
  
  // Si pas d'active, retourner la dernière étape terminée
  const doneSteps = steps.filter(s => s.status === 'done')
  if (doneSteps.length > 0) return doneSteps[doneSteps.length - 1]
  
  // Sinon retourner la première étape
  return steps[0] || { noLabel: '--', title: 'En attente de démarrage', status: 'todo', meta: '' }
})
...
const renderMarkdown = (content) => {
  ...
  // Supprimer le titre H2 du début (## xxx), car le titre de section est déjà affiché à l'extérieur
  let processedContent = content.replace(/^##\s+.+\n+/, '')
  
  // Traiter les blocs de code
  let html = processedContent.replace(/```(\w*)\n([\s\S]*?)```/g, '<pre class="code-block"><code>$2</code></pre>')
  
  // Traiter le code en ligne
  ...
  // Traiter les listes - support des sous-listes
  ...
  // Envelopper les listes non ordonnées
  html = html.replace(/(<li class="md-li"[^>]*>.*?<\/li>\s*)+/g, '<ul class="md-ul">$&</ul>')
  // Envelopper les listes ordonnées
  html = html.replace(/(<li class="md-oli"[^>]*>.*?<\/li>\s*)+/g, '<ol class="md-ol">$&</ol>')

  // Nettoyer les espaces entre les éléments de liste
  html = html.replace(/<\/li>\s+<li/g, '</li><li')
  // Nettoyer les espaces après les balises d'ouverture de liste
  html = html.replace(/<ul class="md-ul">\s+/g, '<ul class="md-ul">')
  html = html.replace(/<ol class="md-ol">\s+/g, '<ol class="md-ol">')
  // Nettoyer les espaces avant les balises de fermeture de liste
  ...
  // Traiter les lignes de séparation
  html = html.replace(/^---$/gm, '<hr class="md-hr">')
  
  // Traiter les sauts de ligne - ligne vide = séparation de paragraphe, saut simple = <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // Nettoyer les balises <br> autour des éléments de bloc
  html = html.replace(/<br>\s*(<ul|<ol|<blockquote)/g, '$1')
  html = html.replace(/(<\/ul>|<\/ol>|<\/blockquote>)\s*<br>/g, '$1')
  // Nettoyer les <p><br> suivis d'éléments de bloc (causés par des lignes vides superflues)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Nettoyer les balises <br> consécutives
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Nettoyer les <br> entre éléments de bloc et balises de paragraphe
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Corriger la numérotation des listes ordonnées non consécutives : quand des <ol> à item unique sont séparés par du contenu
...
const getLogLevelClass = (log) => {
  if (log.includes('ERROR') || log.includes('错误') || log.includes('erreur')) return 'error'
  if (log.includes('WARNING') || log.includes('警告') || log.includes('avertissement')) return 'warning'
  // INFO utilise la couleur par défaut, non marqué comme success
  return ''
}
        ...
        newLogs.forEach(log => {
          ...
          if (log.action === 'section_complete') {
            if (log.details?.content) {
              ...
              // Déplier automatiquement la section venant d'être générée
          ...
          if (log.action === 'report_complete') {
            ...
            currentSectionIndex.value = null  // S'assurer de supprimer l'état de chargement
            emit('update-status', 'completed')
            stopPolling()
            // La logique de défilement est gérée dans le nextTick après la boucle
          }
...
// Extraire le contenu de la réponse finale - extraire le contenu de section depuis la réponse LLM
const extractFinalContent = (response) => {
  ...
  // Tenter de trouver le contenu après Final Answer: (plusieurs formats supportés)
  // Format 1 : Final Answer:\n\ncontenu
  // Format 2 : Final Answer: contenu
  const finalAnswerMatch = response.match(/Final\s*Answer:\s*\n*([\s\S]*)$/i)
  if (finalAnswerMatch) {
    return finalAnswerMatch[1].trim()
  }
  
  // Tenter de trouver le contenu après "最终答案:"
  const chineseFinalMatch = response.match(/最终答案[:：]\s*\n*([\s\S]*)$/i)
  if (chineseFinalMatch) {
    return chineseFinalMatch[1].trim()
  }
  
  // Si commence par ## ou # ou >, c'est probablement du contenu markdown direct
  const trimmedResponse = response.trim()
  if (trimmedResponse.match(/^[#>]/)) {
    return trimmedResponse
  }
  
  // Si le contenu est long et contient du format markdown, tenter de supprimer le processus de réflexion
  if (response.length > 300 && (response.includes('**') || response.includes('>'))) {
    // Supprimer le processus de réflexion commençant par Thought:
...
.section-number {
  ...
  font-size: 16px;
  color: #9CA3AF; /* Gris foncé, ne change pas selon l'état */
  font-weight: 500;
}

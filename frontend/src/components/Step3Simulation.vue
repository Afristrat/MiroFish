  ...
  <div class="simulation-panel">
    ...
    <div class="control-bar">
      <div class="status-group">
        <!-- Progression de la plateforme Twitter -->
        ...
        <!-- Progression de la plateforme Reddit -->
      ...
      <div class="action-controls">
        ...
        >
          ...
          {{ isGeneratingReport ? 'Lancement en cours...' : 'Commencer la génération du rapport' }}
          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-body">
                ...
                <!-- CREATE_COMMENT : Publier un commentaire -->
                ...
                <!-- DO_NOTHING : Aucune action (silencieux) -->
                ...
                <!-- Repli générique : type inconnu ou content non traité par les cas ci-dessus -->
...
const props = defineProps({
  simulationId: String,
  maxRounds: Number, // Nombre maximum de tours transmis depuis Step2
  minutesPerRound: {
    type: Number,
    default: 30 // 30 minutes par tour par défaut
  },
  projectData: Object,
  graphData: Object,
  systemLogs: Array
...
const phase = ref(0) // 0: non démarré, 1: en cours, 2: terminé
const isStarting = ref(false)
const isStopping = ref(false)
const startError = ref(null)
const runStatus = ref({})
const allActions = ref([]) // Toutes les actions (cumul incrémental)
const actionIds = ref(new Set()) // Ensemble d'IDs d'actions pour la déduplication
const scrollContainer = ref(null)

// Computed
// Afficher les actions par ordre chronologique (les plus récentes en bas)
const chronologicalActions = computed(() => {
  return allActions.value
})

// Compteur d'actions par plateforme
...
// Formater le temps écoulé de simulation (calculé selon les tours et minutes par tour)
...
// Temps écoulé de simulation pour la plateforme Twitter
const twitterElapsedTime = computed(() => {
  return formatElapsedTime(runStatus.value.twitter_current_round || 0)
})

// Temps écoulé de simulation pour la plateforme Reddit
...
// Réinitialiser tous les états (pour redémarrer la simulation)
const resetAllState = () => {
  phase.value = 0
  runStatus.value = {}
  allActions.value = []
  ...
  stopPolling()  // Arrêter le polling potentiellement en cours
}
...
const doStartSimulation = async () => {
  ...
  // Réinitialiser tous les états d'abord pour éviter l'impact de la simulation précédente
  resetAllState()
  
  isStarting.value = true
  startError.value = null
  addLog('Lancement de la simulation bi-plateforme en parallèle...')
  ...
  try {
    const params = {
      ...
      platform: 'parallel',
      force: true,  // Forcer le redémarrage
      enable_graph_memory_update: true  // Activer la mise à jour dynamique du graphe
    }
    ...
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('✓ Anciens logs de simulation nettoyés, redémarrage de la simulation')
      }
      addLog('✓ Moteur de simulation lancé avec succès')
      ...
      startError.value = res.error || 'Échec du lancement'
      addLog(`✗ Échec du lancement: ${res.error || 'Erreur inconnue'}`)
      emit('update-status', 'error')
    }
  } catch (err) {
    startError.value = err.message
    addLog(`✗ Exception de lancement: ${err.message}`)
    emit('update-status', 'error')
  } finally {
    isStarting.value = false
  }
...
const handleStopSimulation = async () => {
  ...
  addLog('Arrêt de la simulation en cours...')
  ...
  try {
    ...
    addLog(`Exception d'arrêt: ${err.message}`)
  } finally {
    isStopping.value = false
  }
}
...
// Suivre le dernier tour de chaque plateforme pour détecter les changements et afficher les logs
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Détecter les changements de tour par plateforme et afficher les logs
      ...
      // Détecter si la simulation est terminée (via runner_status ou état des plateformes)
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // Vérification supplémentaire : si le backend n'a pas encore mis à jour runner_status
      // Vérifier via les états twitter_completed et reddit_completed
      const platformsCompleted = checkPlatformsCompleted(data)
      
      if (isCompleted || platformsCompleted) {
        if (platformsCompleted && !isCompleted) {
          addLog('✓ Toutes les simulations de plateformes détectées comme terminées')
        }
...
const checkPlatformsCompleted = (data) => {
  // Si aucune donnée de plateforme, retourner false
  if (!data) return false
  
  // Vérifier l'état de complétion de chaque plateforme
  const twitterCompleted = data.twitter_completed === true
  const redditCompleted = data.reddit_completed === true
  
  // Si au moins une plateforme est terminée, vérifier si toutes les plateformes activées sont terminées
  // Déterminer si une plateforme est activée via actions_count (count > 0 ou running a été true)
  const twitterEnabled = (data.twitter_actions_count > 0) || data.twitter_running || twitterCompleted
  const redditEnabled = (data.reddit_actions_count > 0) || data.reddit_running || redditCompleted
  
  // Si aucune plateforme n'est activée, retourner false
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      // Utiliser all_actions pour obtenir la liste complète des actions
      const serverActions = res.data.all_actions || []
      
      // Ajouter de nouvelles actions de manière incrémentale (déduplication)
      ...
      // Pas de défilement automatique, laisser l'utilisateur parcourir librement
      // Les nouvelles actions sont ajoutées en bas
    }
  } catch (err) {
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog('Demande de génération du rapport envoyée, veuillez patienter...')
    return
  }
  
  isGeneratingReport.value = true
  addLog('Lancement de la génération du rapport...')
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`✓ Tâche de génération du rapport lancée: ${reportId}`)
      
      // Rediriger vers la page du rapport
      router.push({ name: 'Report', params: { reportId } })
    } else {
      addLog(`✗ Échec du lancement de la génération du rapport: ${res.error || 'Erreur inconnue'}`)
      isGeneratingReport.value = false
    }
  } catch (err) {
    addLog(`✗ Exception lors du lancement de la génération du rapport: ${err.message}`)
    isGeneratingReport.value = false
  }
}
...
onMounted(() => {
  addLog('Step3 Initialisation de l'exécution de la simulation')
  if (props.simulationId) {
    doStartSimulation()
  }
})

        ...
        <div class="action-bar-header">
          ...
          <div class="action-bar-tabs">
            ...
            >
              ...
              <span>Converser avec le Report Agent</span>
            </button>
            <div class="agent-dropdown" v-if="profiles.length > 0">
              ...
              >
                ...
                <span>{{ selectedAgent ? selectedAgent.username : 'Converser avec n\'importe quel individu du monde' }}</span>
                <svg class="dropdown-arrow" :class="{ open: showAgentDropdown }" viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2">
                  <polyline points="6 9 12 15 18 9"></polyline>
                </svg>
              </button>
              <div v-if="showAgentDropdown" class="dropdown-menu">
                <div class="dropdown-header">Sélectionner un interlocuteur</div>
                <div 
            ...
            >
              ...
              <span>Envoyer un questionnaire dans le monde</span>
            </button>
          </div>
        </div>
        ...
        <div v-if="activeTab === 'chat'" class="chat-container">
          ...
          <div v-if="chatTarget === 'report_agent'" class="report-agent-tools-card">
            <div class="tools-card-header">
              ...
              <div class="tools-card-info">
                ...
                <div class="tools-card-subtitle">Version dialogue rapide de l'agent de génération de rapports, peut utiliser 4 outils spécialisés, dispose de la mémoire complète de MiroFish</div>
              </div>
              ...
              <div class="tools-grid">
                <div class="tool-item tool-purple">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">Aligne les données réelles de base avec l'état de l'environnement simulé, combine les mécanismes Global/Local Memory pour fournir une analyse d'attribution approfondie transtemporelle</div>
                  </div>
                </div>
                <div class="tool-item tool-blue">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">Algorithme de parcours en largeur basé sur la structure de graphe, reconstruit les chemins de propagation des événements, capture la topologie complète du flux d'informations</div>
                  </div>
                </div>
                <div class="tool-item tool-orange">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">Interface de requête instantanée basée sur GraphRAG, optimise l'efficacité d'indexation, pour extraire rapidement les attributs de nœuds spécifiques et les faits discrets</div>
                  </div>
                </div>
          ...
          <div v-if="chatTarget === 'agent' && selectedAgent" class="agent-profile-card">
            ...
            <div v-if="showFullProfile && selectedAgent.bio" class="profile-card-body">
              <div class="profile-card-bio">
                <div class="profile-card-label">Biographie</div>
          ...
          <div class="chat-messages" ref="chatMessages">
            <div v-if="chatHistory.length === 0" class="chat-empty">
              ...
              <p class="empty-text">
                {{ chatTarget === 'report_agent' ? 'Discutez avec le Report Agent pour approfondir le contenu du rapport' : 'Discutez avec les individus simulés pour connaître leurs points de vue' }}
              </p>
            </div>
            <div 
          ...
          <div class="survey-setup">
            <div class="setup-section">
              <div class="section-header">
                <span class="section-title">Sélectionner les sujets de l'enquête</span>
                <span class="selection-count">{{ selectedAgents.size }} / {{ profiles.length }} sélectionnés</span>
              </div>
...
const chatHistoryCache = ref({}) // Cache de tous les historiques de conversation : { 'report_agent': [], 'agent_0': [], 'agent_1': [], ... }
...
// Sauvegarder l'historique de conversation actuel dans le cache
...
const selectReportAgentChat = () => {
  // Sauvegarder l'historique de conversation actuel
  saveChatHistory()
  ...
  // Restaurer l'historique de conversation du Report Agent
...
const selectAgent = (agent, idx) => {
  ...
  // Restaurer l'historique de conversation de cet Agent
  chatHistory.value = chatHistoryCache.value[`agent_${idx}`] || []
  addLog(`Interlocuteur sélectionné : ${agent.username}`)
}
...
const renderMarkdown = (content) => {
  ...
  // Nettoyer les <p><br> suivis d'éléments de bloc
  ...
  // Corriger la numérotation des listes ordonnées non consécutives
...
const sendMessage = async () => {
  ...
  try {
    ...
    scrollToBottom()
    // Sauvegarder automatiquement l'historique dans le cache
    saveChatHistory()
  }
}

const sendToReportAgent = async (message) => {
  addLog(`Envoi au Report Agent : ${message.substring(0, 50)}...`)
  ...
  if (res.success && res.data) {
    ...
    addLog('Report Agent a répondu')
  } else {
    throw new Error(res.error || 'Échec de la requête')
  }
}
...
const sendToAgent = async (message) => {
  ...
  if (chatHistory.value.length > 1) {
    const historyContext = chatHistory.value
      ...
      .slice(-6)
      .map(msg => `${msg.role === 'user' ? 'Questionneur' : 'Vous'}：${msg.content}`)
      .join('\n')
    prompt = `Voici notre conversation précédente :\n${historyContext}\n\nMa nouvelle question est : ${message}`
  }
  ...
  if (res.success && res.data) {
    // Chemin correct des données : res.data.result.results est un dictionnaire d'objets
    // Format : {"twitter_0": {...}, "reddit_0": {...}} ou mono-plateforme {"reddit_0": {...}}
    const resultData = res.data.result || res.data
    const resultsDict = resultData.results || resultData
    
    // Convertir le dictionnaire d'objets en tableau, priorité aux réponses de la plateforme Reddit
    let responseContent = null
    const agentId = selectedAgentIndex.value
    
    if (typeof resultsDict === 'object' && !Array.isArray(resultsDict)) {
      // Priorité aux réponses de la plateforme Reddit, puis Twitter
...
const submitSurvey = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Convertir le dictionnaire d'objets en format tableau
      ...
      addLog(`${surveyResults.value.length} réponses reçues`)
    } else {
      throw new Error(res.error || 'Échec de la requête')
    }
  } catch (err) {
...
// Lifecycle
onMounted(() => {
  addLog('Step5 Interaction approfondie initialisée')
  loadReportData()
  loadProfiles()
...
/* Left Panel - Report Style (identique à Step4Report.vue) */
.left-panel.report-style {
  width: 45%;
  min-width: 450px;
  background: #FFFFFF;
...
/* Corriger la numérotation des listes ordonnées - utiliser les compteurs CSS pour une numérotation continue */
.message-text {
...
/* Style des citations dans la zone chat/questionnaire */
